...
#         return len(self.names)
"""
#####################################class Vessel_radiomics_4fold_svm(Dataset)feature_num = 54
#####################################class Vessel_radiomics_4fold_svm(Dataset)feature_num = 54
#####################################class Vessel_radiomics_4fold_svm(Dataset)feature_num = 54
"""
...
class Vessel_PCNNradiomics_4fold(Dataset):
    def __init__(self, feature_list,
                 num_points=8192, phase='train',
                 root='./data/EVARnpz',############################################根目录
                 threshold=None, resample=True, category=2,  # 二分类###################################################################################这里是否可以改为多分类
                 scale=0.05, 
        ...
        self.init_data()##############lin chuang shu ju
        self.init_feature(feature_list)
        self.names = np.load('./data/seed{}/{}.npy'.format(self.randomseed, self.phase))  # 名字
        self.names = self.names.tolist()
        # print(len(self.names))###########################################################################################################
        self.names_copy = []
        for item in self.names:
    ...
    def init_data(self):
        randomseed = int(str(self.randomseed)[:2])
        # print(randomseed)
        excel_dir = './data/radiomics_data/seed{}/radiomics_selected{}.xls'.format(randomseed, self.feature_num)  #####################需要注意的是radiomics feature xls# 选中的影像组学特征有几个
        wb = xlrd.open_workbook(excel_dir)
        sh = wb.sheet_by_index(0)
        total_list = {}
            # 获取表格的总行数和总列数
        ...
        for i in tqdm(range(1, sh.nrows)):
            ...
            for j in range(self.feature_num):
                feature_index: str = 'feature' + str(j)
                total_list[str(int(sh.cell(i, 0).value))][feature_index] = sh.cell(i, 4 + j).value
        # print((total_list.keys()))
        for _, _, files in os.walk(self.root):
            break
