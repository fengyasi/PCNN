...
class get_model(nn.Module):
    def __init__(self, num_class=1, use_pcd=True, use_text=True, normal_channel=3 ** 3, feat_channel=25):
        ...
        # 定义融合层，将点云特征和文本特征结合
        self.fusion = nn.Linear(512 + 1024*3, 1024*3)########################2
        # self.fusion = nn.Linear(512 + 1024, 1024)#########################1
    ...
    def forward(self, xyz, feat):
        ...
        x = torch.cat([x1, x2, x3], dim=1)  # torch.Size([B,3072])  # contact point cloud and CFD 

        if self.use_pcd and self.use_text and feat is not None:
            feat = self.mlp(feat)  # 通过MLP处理文本特征#########################################方法2加上第一行和第三行
            x = torch.cat([feat, x], dim=-1) ########################只有这一行是方法1，和pointnet_radiomics一样
            x = self.fusion(x)  # 通过融合层#################################################2
        elif self.use_pcd and (not self.use_text):
            x = x
        elif self.use_text and (not self.use_pcd) and feat is not None:
            feat = self.mlp(feat)# 通过MLP处理文本特征#########################################方法2加上第一行
            x = feat########################只有这一行是方法1
